...
Up_get = 0
Down_get = 0
files = open('Datagg.txt', 'w').close()
while True:
    files = open('Datagg.txt', 'a')
    U_get = U.get_value()
    I_get = I.get_value()
    ...
    if(((Up_get!=0)or(Down_get!=0))and(N_get!=0)):
        ...
        if (Down_get):
            Dir = 0
        print(str(U_get)+","+str(I_get)+","+str(N_get)+","+str(Duty_get)+","+str(Dir))
        files.write(str(U_get)+","+str(I_get)+","+str(N_get)+","+str(Duty_get)+","+str(Dir)+"\n")
        U_old = U_get
        I_old = I_get
        N_old = N_get
        Duty_old = Duty_get
